class EmailController < ApplicationController
  ...
  def correspond
    ...
    if params_posted?(:message)
      ...
      if @message.valid?
        UserMailer.deliver_message(:user => user, :recipient => recipient, :message => @message, :user_url => profile_for(user), :reply_url => url_for(:action => "correspond", :id => user.screen_name))
